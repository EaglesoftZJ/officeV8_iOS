...
#pragma mark - - - 扫描外部View的创建
    ...
    CGFloat light_buttonX = self.frame.size.width/3;
    CGFloat light_buttonY = CGRectGetMaxY(promptLabel.frame) + scanContent_X * 0.5;
    CGFloat light_buttonW = self.frame.size.width/3;
    CGFloat light_buttonH = 30;
    light_button.frame = CGRectMake(light_buttonX, light_buttonY, light_buttonW, light_buttonH);
    light_button.layer.cornerRadius = 15.0f;
    light_button.layer.borderWidth = 0.5f;
    light_button.layer.borderColor = promptLabel.textColor.CGColor;
...
//    CGFloat promptLabel2X = 0;
//    CGFloat promptLabel2Y = CGRectGetMaxY(light_button.frame) + 30;
//    CGFloat promptLabel2W = self.frame.size.width;
//    CGFloat promptLabel2H = 25;
//    promptLabel2.frame = CGRectMake(promptLabel2X, promptLabel2Y, promptLabel2W, promptLabel2H);
//    promptLabel2.textAlignment = NSTextAlignmentCenter;
//    promptLabel2.font = [UIFont boldSystemFontOfSize:13.0];
//    promptLabel2.adjustsFontSizeToFitWidth = YES;
//    promptLabel2.textColor = [[UIColor whiteColor] colorWithAlphaComponent:0.6];
//    promptLabel2.text = @"如果扫描不出二维码，请点击手动输入设备上的码";
//    [self addSubview:promptLabel2];
    
    // 添加闪光灯按钮
//    UIButton *write_button = [[UIButton alloc] init];
//    CGFloat write_buttonX = self.frame.size.width/3;
//    CGFloat write_buttonY = CGRectGetMaxY(promptLabel2.frame) + scanContent_X * 0.5;
//    CGFloat write_buttonW = self.frame.size.width/3;
//    CGFloat write_buttonH = 30;
//    write_button.frame = CGRectMake(write_buttonX, write_buttonY, write_buttonW, write_buttonH);
//    write_button.layer.cornerRadius = 15.0f;
//    write_button.layer.borderWidth = 0.5f;
//    write_button.layer.borderColor = promptLabel.textColor.CGColor;
//    write_button.layer.masksToBounds = YES;
//    [write_button setTitle:@"手动输入" forState:UIControlStateNormal];
//    [write_button setTitleColor:promptLabel.textColor forState:(UIControlStateNormal)];
//    write_button.titleLabel.font = [UIFont systemFontOfSize:17];
//    [write_button addTarget:self action:@selector(write_buttonAction:) forControlEvents:UIControlEventTouchUpInside];
//    [self addSubview:write_button];
...
- (void)write_buttonAction:(UIButton *)button
{
    if (self.writeAction)
    {
        self.writeAction();
